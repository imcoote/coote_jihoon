...
입력 : 첫째 줄에 채널의 수 N이 주어진다. (2 ≤ N ≤ 100)
다음 N개 줄에는 채널의 이름이 한 줄에 하나씩 주어진다. 채널의 이름은 최대 10글자이고, 알파벳 대문자와 숫자로만 이루어져 있다.
이미 KBS1이 첫 번째에, KBS2가 두 번째에 있는 입력은 주어지지 않는다.
출력 : 상근이가 눌러야 하는 버튼을 순서대로 공백없이 출력한다.'''

# 어렵다.
# 상근이는 똑똑하네... 나는 무슨소리인지 모르것는디....

channel_su = int(input())
# 채널들을 리스트로 만들고 리스트의 인덱스값으로 채널들의 위치를 조정 하....자   그래 해야지..
# 이게 되나?
channel = [input() for _ in range(channel_su)]
index1 = channel.index('KBS1')
index2 = channel.index('KBS2')
# 어짜피 처음부터 내려 가야하니까 1이랑 4만 누르면 됨
# 1이 2보다 밑에 있으면 2의 값에 +1을 한번 해줌 => 그래야 순서가 맞음
if index1 > index2:
    index2 +=1

print('1'*index1 + '4'*index1 + '1'*index2 + '4'*index2)
...
if index1 > index2:
    index2 +=1
# 생각해보니 index1보다 2가 컸을 때 마지막에 index2값만큼 '4'를 누르면 또 KBS1보다 KBS2가 위로 감
# => -1 한번 해 줘야함(KBS2가 두번째 자리로 와야하기 때문)
