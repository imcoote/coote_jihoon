...
# 문제 요약:
# 상근이는 자전거를 타면서 일정 거리마다 높이를 측정했다.
# 우리는 **높이가 증가하는 구간(오르막길)**을 찾아 가장 큰 오르막길의 크기를 구해야 한다.

# 음...

list = []
input_count = int(input())
heights = list.append(map(int, input().split()))
...
for i in range(1, input_count):
    if heights[i] > heights[i - 1]:
        current_rise = heights[i] - start_height
    else:
...
for i in range(1, input_count):
    if heights[i] > heights[i - 1]:
        current_rise += heights[i] - heights[i - 1]
    else:
        max_rise = max(max_rise, current_rise)
        current_rise = 0

print(max(max_rise, current_rise))
